...
async function notify(message: Message): Promise<void> {
  ...
  if (shouldSendEmail) {
    tasks.push(
      (async () => {
        // Import nodemailer without requiring @types; use require and tolerate either CJS or ESM default
        // @ts-ignore - suppress missing declaration file error for nodemailer
